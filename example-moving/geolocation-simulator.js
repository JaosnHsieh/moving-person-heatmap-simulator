...
module.exports = (() => {
  ...
  var GeoSim = function(params) {
    ...
    var _watchTimer = null,
      ...
      _speed = (params.speed || 100) / SECONDS_IN_HOUR, // km/second (defaults to 40kmh or 25mph)
      _current = {
        coords: {
          latitude: 0,
          longitude: 0,
    ...
    function nextCoord() {
      ...
      if (_index < _coords.length - 1) {
        ...
        var deltaLat =
            (_coords[_index + 1].latitude - coord.latitude) * KM_IN_DEGREE,
          deltaLon =
            (_coords[_index + 1].longitude - coord.longitude) * KM_IN_DEGREE;
    ...
    self.watchPosition = function(cb, error, options) {
      var sendPos = function() {
        cb(_current);
        _watchTimer = setTimeout(sendPos, UPDATE_INTERVAL);
      };
      sendPos();
    };
